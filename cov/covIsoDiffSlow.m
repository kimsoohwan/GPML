function k = covIsoDiffSlow(hyp, x, z, i, pdx, pdz, f_handles)
...
if i ~= 0
    if pdx == 0
        if pdz == 0
            ...
            % k(x, dz/dz_j)
            k = f_handles.dk_ds(hyp, x, z, i, pdx, pdz) * f_handles.ds_dzj(hyp, x, z, i, pdx, pdz);
        end
    else
        if pdz == 0
            % k(dx/dx_i, z)
            k = f_handles.dk_ds(hyp, x, z, i, pdx, pdz) * f_handles.ds_dxi(hyp, x, z, i, pdx, pdz);
        else
            % k(dx/dx_j, dz/dz_j)
    ...
    if i == 1
        ...
        if pdx == 0
            if pdz == 0
                ...
                k = 2 * f_handles.k(hyp, x, z, i, pdx, pdz);
            end
        else
            if pdz == 0
